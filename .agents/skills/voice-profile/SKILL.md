...
> LLM applies to its own output does. See the repository's [writing-with-AI guidance](../../../.claude/rules/writing-with-ai.md).
...
For PDF inputs, use the repository's [PDF Reading skill](../pdf-reading/SKILL.md) to convert
each document before reading it. Then spawn **one subagent per document** with `context: fork`.
Each reads **only its own file**, writes a ~300-word note to
...
- **Using it to pass a detector.** It is a writing aid, not a laundering step. If a venue wants
  an AI-use statement, make one according to that venue's policy.

## Cross-references

- [Writing-with-AI guidance](../../../.claude/rules/writing-with-ai.md) — readability vs provenance, and the human-readable standard
...
- [PDF Reading](../pdf-reading/SKILL.md) — convert PDFs before reading them; keep one document per note
